...
| `Form1` | `Form1.cs` | Form（partial） | 起動時の初期化（`M_DEPT` 読込・オフライン判定・画面位置）、Pat.csv の読込と画面反映、入力者・診療科の入力チェック、新規作成、担当者文例の取得、テンプレート画面の起動 | `DBConn`, `Env`, `Db`, `AppSettings`, `Logger` |
| `Form1` | `Form1.Agree.cs` | 〃 | 同意書一覧の検索と表示、選択行の表示、登録（INSERT/UPDATE）、論理削除、コピーして作成、印刷用データの組み立て、テンプレートの差し込み | `Db`, `AgreeSql`, `ExcelControl` |
| `Form1` | `Form1.ImportExport.cs` | 〃 | 3 テーブルの CSV エクスポート、CSV インポート（キーで UPDATE/INSERT を振り分けるマージ）、シーケンスの再同期 | `AgreeSql`, `TextFieldParser` |
| `Form1.PatCsvFields` | `Form1.cs` | 入れ子クラス | Pat.csv の先頭行から、アプリで使う項目だけを保持する | — |
| `TmpAgree` | `TmpAgree.cs` | Form | 同意書テンプレート（分類と子テンプレートの 2 階層ツリー）の表示・登録・論理削除・並び替え。選んだテンプレートを `Form1` に差し込む | `Form1`, `DBConn`, `Db`, `AgreeSql` |
| `TmpStaff` | `TmpStaff.cs` | Form | 入力者（医師）ごとの担当者文例（`AGREE_STAFF`）の一覧・登録・物理削除 | `DBConn`, `Db`, `AgreeSql` |
...
| `Db` | `Db.cs` | static | OleDb 接続を Open → 実行 → Close する（例外時も必ず Close）。`M_USR` の氏名を 1 件取る | `Env` |
...
- **DB 接続**：`Form1`・`TmpAgree`・`TmpStaff` がそれぞれ `DBConn.GetOpenDBConn()` で自分の `OleDbConnection` を持つ。開閉は操作ごと（`Db` が担当）。CSV 入出力だけは `Form1.ImportExport.cs` が自分で開閉する。
