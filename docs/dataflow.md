...
`"行,列"→値` のDictionaryを構築する。患者情報(3行)・診療科(5行)に加え、**医師ID/氏名は起動時(`readPatCsv`)に読込済みのメモリ上 `patCont[9]/[10]` を再利用**（Pat.csv が存在する場合。無ければ `dr_id`/`dr_name` をフォールバック使用）し、術眼・病名・麻酔・説明・項目1〜4を27〜35行へ割当てる。
