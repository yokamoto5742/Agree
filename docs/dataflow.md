...
医師の同意書システム（眼科）における患者情報の読込からExcel同意書生成までのデータフローを、実装に沿って整理する。
...
2. その患者IDでOracle照会      → 氏名(正)・既存同意書一覧    Form1.cs:1074-1151
...
正確な順序は **「pat.csv（起点）→ Oracle照会 → テンプレを開く → 共通情報書込 → バーコード挿入 → 別名保存」** である。
...
### 4-5. テンプレを開いてセル書込 (`MakeEyeAgree`/`Open` ExcelControl.cs:94,51)
...
当初想定していたフローとの主な相違点は以下の2点。

- **Pat.csv と Oracle の順序**：Pat.csv が起点（患者IDの供給源）であり、その患者IDをキーにOracleを照会する。Oracleが先ではない。
- **ファイルの「開く」と「改名」**：開くのは原本テンプレート（`EyeAgree.xlsm`）。改名は「書込・バーコード挿入の後」の`SaveAs`時点で行われる。「ファイル名を変更して開く」ではなく「開いてから別名で保存」が正しい。
