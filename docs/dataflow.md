...
`"行,列"→値` のDictionaryを構築する。患者情報(3行)・診療科(5行)に加え、**入力者ID/氏名は選択中の同意書のDB値(`AGREE.DR`)を `dr_id`/`dr_name` 経由でB7/B11へ出力**する（一覧選択時に `showPlan` がセットする。Pat.csv の操作者情報は使わないため、誰が印刷しても同じ入力者になる）。術眼・病名・麻酔・説明・項目1〜4を27〜35行へ割当てる。
...
共通情報シートのセル値（患者9桁+科3桁+医師5桁+日付+時刻）と**文書コード5桁**から**36桁のCODE128-C値**を組立て`[11,2]`に書込む。文書コードは旧B4セルではなく`EyeAgreeSettings.ini`の`[BARCODE_SETTINGS] DOCUMENT_CODE`（既定39911）から取得する（`loadBarcodeSettings`）。さらに「共通情報」以外の全フォームシートをループし、`Barcode128`でPNG画像を生成してD1セルに貼付ける (ExcelControl.cs:243-321)。**保存より前**に行うことで保存ファイルに画像を残す。
